import React from 'react';
import { NavLink } from 'react-router-dom';
import styled from 'styled-components';
import App from '../App.js'

function WelcomePage(props) {
  console.log(props, "WelcomePage Props");

    const UserSideDiv = styled.div`
        width: 25%;
        margin: 2rem;
        display: flex;
        flex-direction: column;
        margin: 2rem;
        margin-left: 0.08rem;`

    const UserCatBody = styled.div`
        background-color: white;
        border-radius: 2rem;
        color: orange;
        height: 28rem;
        width: 60%;
        margin: 1rem;
        margin-left: 18rem;
        margin-top: 1.6rem;
        position: absolute;
        top: 39%;
        display: flex;
        flex-direction: row;
        flex-wrap: wrap;
        text-align: center;
        `

    const OtherUserCatBody = styled.div`
        background-color: white;
        border-radius: 2rem;
        color: orange;
        height: 20rem;
        width: 60%;
        margin: 1rem;
        margin-left: 18rem;
        margin-top: 1.6rem;
        position: absolute;
        top: 39%;
        display: flex;
        flex-direction: row;
        flex-wrap: wrap;
        text-align: center;
        top: 135%;`

    const CustomH1 = styled.h1`
        margin-bottom: 2rem;
        `

    const img = "data:image/png;base64,";

  return (
    <div className="App">
        <div className="user-topnine">
            <div className="user-cat">
                    <p>My Top Nine:</p>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/accessories'>Accessories</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/animals'>Animals</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/boardgames'>Board Games</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/books'>Books</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/cars'>Cars</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/food'>Food</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/movies'>Movies</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/music'>Music</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/places'>Places</NavLink>
                    <NavLink className="MiniNavLink" activeClassName="active" to='/videogames'>Video Games</NavLink>      
            </div>

            <CustomH1>My Top Nine</CustomH1>
            <UserSideDiv className="user-sidebar">
                <img className="userPic" src={img} alt="user img" />
                <h2 user={props.UserName}>{props.user || 'User Name'}</h2>
            </UserSideDiv>

            <UserCatBody>
                <div className="useritem">
                    <div className="items accessoryitem">
                        <h4>Accessory</h4>
                        <h6>{props.chosenItems.accessories}</h6>
                    </div>

                    <div className="items animalitem">
                        <h4>Animal</h4>
                        <h6>{props.chosenItems.animals}</h6>
                    </div>

                    <div className="items boardgameitem">
                        <h4>Board Game</h4>
                        <h6>{props.chosenItems.boardGames}</h6>
                    </div>

                    <div className="items bookitem">
                        <h4>Book</h4>
                        <h6>{props.chosenItems.books}</h6>
                    </div>

                    <div className="items caritem">
                        <h4>Car</h4>
                    <h6>{props.chosenItems.cars}</h6>
                    </div>

                    <div className="items fooditem">
                        <h4>Food</h4>
                        <h6>{props.chosenItems.food}</h6>
                    </div>

                    <div className="items movieitem">
                        <h4>Movie</h4>
                        <h6>{props.chosenItems.movies}</h6>
                    </div>

                    <div className="items musicitem">
                        <h4>Music</h4>
                        <h6>{props.chosenItems.music}</h6>
                    </div>

                    <div className="items placeitem">
                        <h4>Place</h4>
                        <h6>{props.chosenItems.places}</h6>
                    </div>

                    <div className="items gameitem">
                        <h4>Video Game</h4>
                        <h6>{props.chosenItems.videoGames}</h6>
                    </div>
                </div>
            </UserCatBody>

            <div className="other-user-topnine">
                <div className="other-user-cat">
                    <p>Their Top Nine:</p>
                    <NavLink className="MiniNavLink" to='/accessories/:id'>Accessories</NavLink>
                    <NavLink className="MiniNavLink" to='/animals/:id'>Animals</NavLink>
                    <NavLink className="MiniNavLink" to='/boardgames/:id'>Board Games</NavLink>
                    <NavLink className="MiniNavLink" to='/books/:id'>Books</NavLink>
                    <NavLink className="MiniNavLink" to='/car/:id'>Cars</NavLink>
                    <NavLink className="MiniNavLink" to='/food/:id'>Food</NavLink>
                    <NavLink className="MiniNavLink" to='/movies/:id'>Movies</NavLink>
                    <NavLink className="MiniNavLink" to='/music/:id'>Music</NavLink>
                    <NavLink className="MiniNavLink" to='/places/:id'>Places</NavLink>
                    <NavLink className="MiniNavLink" to='/videogame/:id'>Video Games</NavLink>      
                </div>

                    <h1>Their Top Nine</h1>
                <OtherUserCatBody>
                </OtherUserCatBody>
            </div>
        </div>
    </div>
    )
}

export default WelcomePage;
